...
Cost is dominated by the context each validator carries, not by how many validators there are: every request a lane makes re-sends everything it holds. This cuts both ways. Handing a lane reconnaissance it would otherwise repeat is a saving, and several lanes working from one shared, hard-won context beat several lanes each rebuilding it. Handing a lane a transcript it does not need is a charge on every request it will make. The fields listed above exist so a lane can be briefed precisely; reach for a fork when the brief cannot carry what the lane actually needs. See `AGENTS.md` ("Why these commands are shaped this way") for the same reasoning applied to reads and long-running commands.
