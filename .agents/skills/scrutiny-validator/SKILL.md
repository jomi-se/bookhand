...
5. **Use feature-reviewer lanes when useful**
   ...
   - Prefer passing that brief as written content. A lane carries whatever
     context it starts with through every request it makes, so forking your
     conversation into it pays off only when the lane genuinely needs what you
     have already worked out and would otherwise rebuild it. When the brief
     above is sufficient on its own, it is both cheaper and sharper.
